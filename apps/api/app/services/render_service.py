...
def _input_args(input_path: str, input_media_kind: str, output_format: str) -> list[str]:
    # Loop still images only when a caller explicitly asks for a video output.
...
def _compact_ffmpeg_error(stderr: str) -> str:
    ...
    # FFmpeg prints banner/configuration noise before the actionable diagnostics.
    return "\n".join(lines[-16:])
...
def render_file(request: RenderRequest) -> RenderJobResponse:
    ...
    render_duration_seconds = round(time.perf_counter() - started_at, 2)
